...
func LoadConfig(path string) (config Config, err error) {
	...
	viper.AutomaticEnv()
// Explicitly bind env vars that have no default, so AutomaticEnv
	// actually picks them up (viper only auto-detects env vars for
	// keys it already knows about via SetDefault or an explicit bind).
